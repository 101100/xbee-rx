...
module.exports = function xbeeRxLibrary(options) {
    ...
    function _sendFrameStreamResponse(frame, timeoutMs, responseFrameType) {
        ...
        //     .take(1)
        //     .do(function (frame) { console.log(frame); })
        //     .publish();
        ...
        resultStream = new rx.ReplaySubject(2);
        ...
        frameSource
            ...
            .subscribe(
                function (a) { resultStream.onNext(a); },
                function (b) { resultStream.onError(b); },
                function () { resultStream.onCompleted(); }
            );
    ...
    function remoteCommand(settings) {
        ...
        if (settings.destinationId) {
            test = _lookupByNodeIdentifier(settings.destinationId, timeoutMs)
    ...
    if (debug) {
        frameSource.subscribe(function (frame) {
