...
module.exports = function xbeeRxLibrary(options) {
    ...
    function closeEverything() {
        ...
        frameSourceConnection.unsubscribe();
    ...
    function _sendFrameStreamResponse(frame, timeoutMs, responseFrameType) {
        ...
        resultStream = frameSource
            .pipe(rx.operators.filter(_createFilter(frameId, responseFrameType)),
                  rx.operators.takeUntil(rx.timer(timeoutMs)));
        ...
        return rx.merge(sendStream, resultStream).pipe(
            rx.operators.timeoutWith(timeoutMs, rx.throwError(new Error("Timed out after " + timeoutMs + " ms")))
        );
    }
    ...
    function _localCommand(command, timeoutMs, commandParameter) {
        ...
        return _sendFrameStreamResponse(frame, timeoutMs, xbee_api.constants.FRAME_TYPE.AT_COMMAND_RESPONSE).pipe(
            rx.operators.map(function(frame) {
    ...
    function remoteCommand(settings) {
        ...
        return _generateDestination(settings, timeoutMs).pipe(
            rx.operators.flatMap(function(destination) {
    ...
    function monitorIODataPackets() {
        return frameSource.pipe(
            rx.operators.filter(function (frame) { return frame.type === xbee_api.constants.FRAME_TYPE.ZIGBEE_IO_DATA_SAMPLE_RX; })
        );
    }
    ...
    function monitorTransmissions() {
        return frameSource.pipe(
            rx.operators.filter(function (frame) { return frame.type === xbee_api.constants.FRAME_TYPE.ZIGBEE_RECEIVE_PACKET; })
        );
    }
    ...
    frameSource = rx.fromEvent(xbeeAPI, "frame_object").pipe(rx.operators.publish());
