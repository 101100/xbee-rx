...
module.exports = function xbeeRxLibrary(options) {
    ...
    function monitorIODataPackets() {
        return frameSource.pipe(
            rx.operators.filter(R.propEq("type", xbee_api.constants.FRAME_TYPE.ZIGBEE_IO_DATA_SAMPLE_RX))
        );
    }
    ...
    function monitorTransmissions() {
        return frameSource.pipe(
            rx.operators.filter(R.propEq("type", xbee_api.constants.FRAME_TYPE.ZIGBEE_RECEIVE_PACKET))
        );
    }
